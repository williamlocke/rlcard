...
class HeartsEnv(Env):
    ...
    def custom_features(self, state):
        ...
        # features[0] = suits[target]
        # current_winner = self.current_winner(trick)
        # num_winners = 0
        # for c in player_hand:
        #     if get_rank(c) > get_rank(current_winner) and c[0] == current_winner[0]:
        #         num_winners += 1
        #features[1] = num_winners
        ...
        #         features[2] = 1
        #     elif has_hearts and len(player_hand) > 1 and not all([c[0] == 'H' for c in player_hand]) and \
        #                 any([c[0] == 'H' and c[1] in ['A', 'K', 'Q', 'J'] for c in player_hand]):
        #         features[3] = 1
        #     elif not has_hearts and len(player_hand) > 1 and any([c[1] in ['A', 'K', 'Q', 'J'] for c in player_hand]):
        #         features[4] = 1
